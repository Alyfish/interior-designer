...
def process_with_yolo(image, model):
    ...
    if image is None or not hasattr(image, 'shape'):
        print("Warning: Invalid image provided to process_with_yolo")
        # Return empty results
        import types
        empty_results = types.SimpleNamespace()
        empty_results.masks = None
        empty_results.boxes = None
        empty_results.orig_shape = (0, 0)
...
def detect_walls_and_floors(image):
    ...
    if image is None or image.size == 0:
        print("Warning: Empty image provided to detect_walls_and_floors")
        return [], []
...
def create_yolo_objects(results, image):
    ...
    # Check if image is valid
    if image is None or not hasattr(image, 'shape'):
        print("Warning: Invalid image provided to create_yolo_objects")
        return objects
