...
st.set_page_config(layout="wide", page_title="Interactive Interior Segmentation Tool")

# Add custom CSS
st.markdown("""
    <style>
        #MainMenu, footer, .stDeployButton, [data-testid="stToolbar"], .stApp > header {
            visibility: hidden;
        }
        .stApp { 
...
@st.cache_resource
def load_sam_model():
    """Load SAM model if available, otherwise return None"""
    try:
        ...
        st.warning(f"SAM model not available: {e}. Proceeding with YOLO-only mode.")
        return None
...
def main():
    ...
    # Set up page layout with sidebar
    st.sidebar.title("Interior Segmentation")
    st.sidebar.markdown("""
    ### Instructions
    1. Upload an image of an interior room
    2. Hover over objects to see their labels
    3. Click to select objects for further processing
    4. Right-click to cycle through overlapping objects
    5. Use "Draw to Detect" to add custom objects
    
    **Selection Tip:** When objects overlap with walls, 
    right-click to select the furniture instead of the wall.
    """)
    
    # Main content
    st.title("Interactive Interior Segmentation Tool")
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.spinner('Loading models...'):
                ...
                # Try to load SAM model (optional)
            ...
            with st.spinner('Detecting objects...'):
                try:
                    # Get YOLO results
                    results = process_with_yolo(image, yolo_model)
                    
                    # Create object maps from YOLO with improved wall detection
                    objects = create_yolo_objects(results, image)
                    st.session_state.objects = objects
                    
                    # Create interactive HTML
                    html_content = create_interactive_html(image, objects)
                    
                    # Display the interactive visualization
                    st.components.v1.html(
                        html_content, 
                        height=min(image.shape[0] * 1.2, 1000),
                        scrolling=True
                    )
                    
                    # Display object counts summary
    ...
    # Instructions if no file uploaded
        ...
        - **Detect objects** in interior room images using YOLOv8
        - **Select and interact** with detected objects
        - **Add custom objects** using the Draw to Detect tool
        - **Export selections** for further processing
        
        Try it by uploading your own image!
        """)
