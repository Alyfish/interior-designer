...
class ObjectDetector:
    ...
    def detect_objects(self, image_path: str) -> tuple[np.ndarray, list, str]:
        """Enhanced detect_objects method that combines YOLO and SAM results"""
        try:
            ...
            # Load and log image size
            original_image_cv = cv2.imread(image_path)
            if original_image_cv is None:
                raise ValueError(f"Could not read image: {image_path}")
            
            img_height, img_width = original_image_cv.shape[:2]
            logger.info(f"🖼️  Processing image: {img_width}x{img_height} in object detector")

            # Get YOLOv8 detections first
            yolo_results = self.model(image_path, verbose=False)
            ...
            logger.info(f"✅ YOLOv8 detected {len(yolo_objects)} objects")

            # Get wall and floor detections
            wall_floor_objects = self.detect_walls_and_floors(original_image_cv)
            # Combine YOLO with walls and floors before SAM
            yolo_objects.extend(wall_floor_objects)

            # Step 3: SAM detection with timeout handling
            logger.info(f"Starting SAM detection (this may take up to {sam_timeout_minutes} minutes)...")
            
            sam_objects = []
            if self.use_sam:
                sam_objects = self.sam_detector.run_sam_detection(image_path, sam_timeout_minutes)
                
                if sam_objects:
                    logger.info(f"✅ SAM detected {len(sam_objects)} semantically labeled objects")
                else:
                    logger.warning("SAM detection failed or timed out, continuing with YOLO + walls/floors only")
            else:
                logger.info("SAM detection disabled")

            # Combine and merge results
            detected_objects = self._merge_detections(yolo_objects, sam_objects)
            logger.info(f"✅ Combined detection: {len(detected_objects)} total objects")

            # Draw segmentation on a copy of the original image
            segmented_image = self.draw_segmentation_masks(original_image_cv, detected_objects)
    ...
    def draw_segmentation_masks(self, image, objects, draw_labels=True, draw_boxes=True, alpha=0.4):
        """
        Draws segmentation masks, bounding boxes, and labels on the image.

        Args:
            image (np.ndarray): The input image.
            objects (List[Dict]): A list of detected objects.
            draw_labels (bool): Whether to draw class labels.
            draw_boxes (bool): Whether to draw bounding boxes.
            alpha (float): The opacity for the segmentation masks.
        ...
        # Sort objects to draw smaller ones on top, which is better for visualization
        sorted_objects = sorted(objects, key=lambda x: x.get('area', 0) or (x['bbox'][2]-x['bbox'][0])*(x['bbox'][3]-x['bbox'][1]), reverse=True)
        ...
        for obj in sorted_objects:
            ...
            # Generate a consistent color for each class for visualization
            class_name = obj.get('class', 'unknown')
            color_seed = sum(ord(c) for c in class_name)
            rng = np.random.default_rng(color_seed)
            color = rng.integers(50, 200, size=3).tolist() # Use a range to avoid very dark/light colors

            np_contours = [np.array(c).astype(np.int32) for c in contours]
            
            # Draw the filled contour on the overlay
            cv2.drawContours(overlay, np_contours, -1, color, -1)

        # Blend the overlay with the original image
        output_image = cv2.addWeighted(overlay, alpha, image, 1 - alpha, 0)

        # Draw boxes and labels on top of the blended image for clarity
        if draw_boxes or draw_labels:
            for obj in objects:
                ...
                # Use a consistent color for boxes and text for a cleaner look
                box_color = (50, 255, 50) # Bright green
                ...
                if draw_labels:
                    ...
                    # Put text above the box with a solid background
                    (text_w, text_h), baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
                    
                    # Adjust text position to ensure it's inside the image frame
                    text_y = y1 - 10
                    if text_y - text_h < 0:
                        text_y = y1 + text_h + baseline + 10

                    cv2.rectangle(output_image, (x1, text_y - text_h - baseline), (x1 + text_w, text_y + baseline), box_color, -1)
                    cv2.putText(output_image, label, (x1, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
